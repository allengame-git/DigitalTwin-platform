...
#### 設施導覽模組 (Facility Navigation Module) — 2026-03-03 新增

獨立的 3D 設施導覽頁面，使用者上傳建築物/設施 GLB/glTF 模型，在互動 3D 場景中瀏覽。

- **多層巢狀場景**: 支援場區 → 建築 → 樓層 → 房間等多層級場景切換，切換時淡出淡入 + fly-to 動畫。
- **GLB/glTF 模型上傳**: 拖拉上傳，單檔最大 100MB，模型自帶定位 + 場景內 Gizmo 微調。
- **模型互動**: Hover 高亮 (emissive)、點擊選取、Tooltip 名稱顯示。
- **Rich Content 資訊面板**: 點擊模型彈出右側 InfoPanel，支援四種類型：
  - `TEXT` — 文字說明
  - `IMAGE` — 圖片展示 + Lightbox 放大
  - `DOCUMENT` — 檔案下載連結 (PDF/DOC/XLS)
  - `LINK` — 外部超連結
- **編輯模式**: TransformControls (translate/rotate/scale) + 數值精確輸入面板，debounce 存後端。
- **側邊欄導覽**: 麵包屑導覽 (BreadcrumbNav)、場景樹 (SceneTree)、模型清單、2D 平面圖 (PlanView)。
- **2D 平面圖**: 手動上傳或自動俯視截圖，模型位置映射為可點擊標記。
- **地形支援**: CSV (x, y, elevation) 上傳 → Python 插值 → heightmap + hillshade，可疊加衛星影像紋理。
- **座標偏移**: 支援 coordShiftX/Y/Z + rotation 偏移設定，對齊地形與模型座標空間。
- **資料管理整合**: DataManagementPage 內含場景管理、模型上傳、Rich Content 編輯、地形上傳四個 Tab。
...
│   │   │   ├── FacilityToolbar.tsx        # 工具列
│   │   │   ├── TransformInputPanel.tsx    # 精確數值輸入
│   │   │   └── CoordShiftPanel.tsx        # 座標偏移設定
│   │   ├── overlay/            # UI 疊加層 (LayerPanel, ClippingTool, BoreholeDetail)
│   │   ├── data/               # 資料上傳元件
│   │   │   ├── FacilityUploadSection.tsx  # 設施上傳管理 (4 Tab)
...
- [x] **設施導覽模組 (2026-03-03)**:
  - **架構**: 獨立 3D 頁面 (`/project/:code/facility`)，單一 Canvas + 動態載入
  - **後端**: `server/routes/facility.ts` (805 行)，完整 CRUD — 場景/模型/Rich Content/平面圖/地形
  ...
  - **3D 場景**: FacilityCanvas (R3F) + Environment + CameraController (fly-to) + Terrain (heightmap displacement)
  - **模型互動**: FacilityModelItem — `useGLTF` 載入、hover emissive 高亮、click 選取、TransformControls 編輯
  - **導覽 UI**: 側邊欄 (BreadcrumbNav + SceneTree + 模型清單)、2D PlanView (模型標記)
  - **Rich Content**: InfoPanel — TEXT/IMAGE (Lightbox)/DOCUMENT (下載)/LINK
  - **編輯工具**: Toolbar (編輯模式切換)、TransformInputPanel (精確數值)、CoordShiftPanel (座標偏移)
  - **上傳管理**: FacilityUploadSection (4 Tab: 場景管理/模型上傳/資訊編輯/地形上傳)
  - **導覽連結**: GeologySidebar 新增「設施導覽」快捷跳轉按鈕
