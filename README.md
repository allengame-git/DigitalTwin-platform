...
#### 設施導覽模組 (Facility Navigation Module) — 2026-03-05
...
- **動畫系統** (2026-03-05):
  ...
  - **時間軸編輯器**: 底部面板，含動畫清單、播放控制（播放/暫停/停止）、時間軸軌道（關鍵幀菱形標記 + 紅色播放指示線）、屬性編輯（名稱/時長/觸發方式/循環/緩動曲線）。
...
- **側邊欄**: 白色亮色主題，含返回儀表板連結、可收合切換、BreadcrumbNav、SceneTree、模型清單（含多選 checkbox + 全域播放按鈕）、2D PlanView。
...
│   │   ├── facilityStore.ts    # 設施導覽 (場景/模型/多選/批次操作/編輯/動畫播放控制)
...
| keyframes    | Json    | 關鍵幀陣列 [{time, position, rotation, scale}] |
| sortOrder    | Int     | 排序                                      |
