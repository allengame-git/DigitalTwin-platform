...
#### 設施導覽模組 (Facility Navigation Module) — 2026-03-04
...
- **模型資訊面板**: 點擊模型後右下角浮動面板（340px，60vh），分三區塊顯示：
  ...
  - 無資料的區塊自動隱藏
- **截圖功能**: 底部右下角截圖按鈕，使用 File System Access API（`showSaveFilePicker`）彈出 OS 原生存檔對話框，不支援時 fallback 自動下載。
...
- **WYSIWYG Editor**: [TipTap 2.x](https://tiptap.dev/) (`@tiptap/react`, `@tiptap/starter-kit`, `@tiptap/extension-link`)
...
│   │   │   └── RichTextView.tsx           # WYSIWYG HTML 唯讀渲染
...
- [x] **設施模型資訊系統 (2026-03-04)**:
  ...
  - **WYSIWYG 編輯器**: TipTap 2.x 共用元件 `RichTextEditor`（Bold/Italic/H2/H3/BulletList/Link 工具列）
