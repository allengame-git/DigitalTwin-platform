...
- **2D 平面圖**: 手動上傳，模型位置映射為可點擊標記。Lobby 場景左下角顯示平面圖開啟按鈕。
- **平面圖標記編輯 (2026-03-10)**: 浮動平面圖右上角「編輯標記」toggle，開啟後：
  - 拖曳標記到平面圖上的正確位置（百分比座標，獨立於 3D position）
  - 每個標記旁眼睛按鈕可切換可見性（隱藏不需要的標記）
  - decorative 模型不顯示在平面圖上
  - 拖曳結束 debounce 300ms 自動存入 DB（`planX`/`planY`/`planVisible`）
  - sidebar PlanView 同步讀取自訂位置與可見性篩選
- **地形支援 (2026-03-10)**:
  - CSV → Python 插值 → 16-bit heightmap + hillshade + 衛星影像（rasterio reproject 對齊 DEM，輸出 JPEG）
  - **三種紋理模式**: 衛星影像 / 山影圖 / 色階（Color Ramp），sidebar 即時切換
  - **色階模式**: `onBeforeCompile` GLSL shader 注入，支援 5 種 Color Ramp + 反轉 + 自訂高程範圍
  - **地形顯示開關**: sidebar 地形設定區眼睛按鈕，可即時開關地形渲染
- **框選多選 (Marquee Selection, 2026-03-10)**: 編輯/動畫模式下左鍵拖曳框選矩形範圍內模型，CMD/Ctrl 追加選取
- **動畫匯出/匯入 (2026-03-10)**: 匯出單一/全部動畫為 JSON，匯入時自動位置偏移對齊，支援多模型批次匯入
- **Canvas ErrorBoundary (2026-03-10)**: 3D 場景載入失敗不再白屏，顯示錯誤訊息 + 重新載入按鈕
- **跨平台 Python 路徑 (2026-03-10)**: `server/lib/pythonPath.ts` 自動偵測 venv（macOS/Linux/Windows）
- **獨立資料管理頁面** (`/project/:code/facility-data`)：5 個 Tab
  - 場景管理：N 級遞迴場景樹、每層節點可新增子場景、模型下拉正確顯示父場景模型、設定相機預設位置、場景範圍設定
  ...
  - 場景地形：CSV + 衛星影像上傳、地形參數管理（shift/rotation 編輯、刪除）
...
│   │   │   ├── AnimationTimeline.tsx      # 動畫時間軸面板（播放控制/關鍵幀編輯/匯出匯入）
...
│   │   │   ├── ErrorBoundary.tsx          # 通用 ErrorBoundary（含 FacilityCanvas fallback）
│   │   ├── data/               # 資料上傳元件
│   │   │   ├── FacilityUploadSection.tsx  # 設施上傳管理 (5 Tab，含 N 級遞迴場景樹)
│   │   │   ├── ImageryUploadSection.tsx   # 航照圖上傳管理（從 DataManagementPage 抽出）
│   │   │   ├── GeologyModelSection.tsx    # 3D 地質模型上傳管理（從 DataManagementPage 抽出）
│   │   │   ├── GeophysicsUploadSection.tsx # 地球物理探查上傳管理（從 DataManagementPage 抽出）
│   │   │   ├── DataPageTOC.tsx            # 浮動目錄導航（IntersectionObserver scroll spy）
...
│   │   └── ...
│   ├── styles/
│   │   └── data-management.css # 資料管理共用 CSS（dm-* classes，兩頁共用）
│   ├── types/
│   │   ├── facility.ts         # 設施模組型別
│   │   └── ...
...
│   │   ├── safePath.ts         # 路徑穿越防護
│   │   └── pythonPath.ts       # 跨平台 Python venv 路徑解析
│   ├── scripts/
...
- [x] **地質資料管理頁面 UI 重構 (2026-03-09)**:
  - **浮動 TOC + Scroll Spy**: 左側 sticky 導航條（200px），IntersectionObserver 追蹤當前 section，`< 1400px` 自動隱藏
  - **Section 收合**: 每個區塊可折疊，收合時顯示 icon + 標題 + 統計 badge + 展開箭頭
  - **Sub-component 抽取**: 航照圖（`ImageryUploadSection`）、3D 地質模型（`GeologyModelSection`）、地球物理（`GeophysicsUploadSection`）三個 inline 區塊抽為獨立元件，DataManagementPage 從 ~1894 行降至 ~1060 行
  - **群組色帶**: 4 群組（基礎設定 slate / 地質資料 amber / 地表資料 cyan / 模型資料 violet）左側 3px border 區分
  - **統計 Badge**: 每個 section header 右側顯示資料筆數
  - **字體升級**: DM Sans（標題/UI）+ JetBrains Mono（座標/mono），Google Fonts 引入
  - **配色微調**: `--bg-page: #f1f5f9`、`--text-primary: #0f172a`，增加卡片對比度

- [x] **設施資料管理頁面 UI 統一 (2026-03-09)**:
  - **共用 CSS 設計系統**: 將 DataManagementPage 的 inline `<style>` 抽取為獨立 `src/styles/data-management.css`（703 行），DataManagementPage 與 FacilityDataPage 共同 import
  - **FacilityDataPage 重構**: 新增 `dm-toc` 左側 sidebar 導航（5 Tab + Lucide icons），匹配地質頁面的 header/layout 風格
