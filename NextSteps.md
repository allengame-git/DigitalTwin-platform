...
**最後更新**: 2026-03-05（設施動畫系統 — 關鍵幀引擎 + 時間軸 UI + GLB 動畫偵測）
...
```
進入場景 → 掃描所有模型動畫
  ├─ trigger='auto' → useFrame 內根據 clock.elapsedTime 自動播放
  └─ trigger='manual' → 等待使用者觸發

useFrame 每幀：
  ├─ GLB mixer.update(delta) → 驅動 AnimationAction
  └─ 關鍵幀插值 → 更新 groupRef position/rotation/scale
```

**時間軸 UI**（`AnimationTimeline.tsx`）：
- 左側：動畫清單（當前模型的所有動畫）
- 右側上方：屬性列（名稱/時長/觸發/循環/緩動）
- 右側中間：播放控制 + TimelineTrack（關鍵幀菱形標記 + 紅色 scrubber）
- 右側下方：新增/刪除關鍵幀按鈕
...
### A1. 動畫編輯模式中使用 TransformControls 記錄關鍵幀

**目前狀態**: 時間軸 UI 可新增關鍵幀，但記錄的是模型的「靜態 DB position/rotation/scale」，而非使用者在場景中即時擺放的位置。

**需實作**:
1. 在動畫模式下，選取模型後顯示 `TransformControls`（類似編輯模式）
2. 拖曳 TransformControls 調整模型位置時，**不存回 DB**（與編輯模式不同），而是暫存到 local state
3. 點擊「+ 新增關鍵幀」時，讀取 `groupRef.current` 的即時 position/rotation/scale 作為關鍵幀資料
4. 時間軸 scrub 到某個關鍵幀時，模型自動跳到該關鍵幀的 position/rotation/scale

**相關檔案**:
- `src/components/facility/FacilityModelItem.tsx` — TransformControls 條件改為 `isEditing || isAnimationEditing`
- `src/components/facility/AnimationTimeline.tsx` — handleAddKeyframe 改為讀取 3D 場景即時 transform
- `src/stores/facilityStore.ts` — 可能需要新增 `animatingModelId` 狀態

**關鍵注意**: 動畫模式的 TransformControls 修改**不能**觸發 `updateModelTransform`（不要存 DB），只用於視覺擺放 + 記錄關鍵幀。
...
| 9 | 動畫編輯需 TransformControls 整合 | 高 | 目前新增關鍵幀只記錄 DB 靜態位置，應記錄場景中即時擺放位置（待 A1） |
| 10 | 手動觸發動畫無播放入口 | 中 | trigger='manual' 的動畫目前無法在導覽模式下觸發（待 A2） |
...
| `FacilityModelItem.tsx` | 單一 GLB：useGLTF + emissive hover + TransformControls + **AnimationMixer + 關鍵幀插值** |
| `FacilityTerrain.tsx` | 16-bit heightmap → PlaneGeometry 頂點置換 + 衛星紋理 |
| `FacilitySidebar.tsx` | 側邊欄整合（亮色主題，含編輯/動畫模式按鈕） |
...
| `AnimationTimeline.tsx` | **動畫時間軸面板**（動畫清單/屬性/播放控制/關鍵幀編輯） |
...
| 檔案 | 說明 |
|:-----|:-----|
| `src/stores/facilityStore.ts` | **設施導覽**（場景樹/模型管理/編輯模式/動畫模式/播放控制/關鍵幀 CRUD） |
...
1. 確認 `animationMode = true`（sidebar 底部按鈕已變紫色）
2. 確認已選取模型且已 `fetchAnimations`
3. 關鍵幀動畫：確認 `keyframes` 陣列非空，且 `duration > 0`
4. GLB 內嵌動畫：確認 `gltfClipName` 與 GLB 中的 AnimationClip 名稱完全匹配
5. 確認 `trigger` 設定：auto 自動播放，manual 需手動觸發
6. Console 檢查 `facilityStore.animations` 是否有資料
