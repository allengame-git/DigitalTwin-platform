...
**最後更新**: 2026-03-06（曲線路徑動畫 + 3D 路徑編輯器 + keyframe 拖曳 + N8 UX 改善）
...
```
getGlobalCurve(posKeyframes)     → 建一條全域 CatmullRomCurve3 (centripetal)
    └─ getLengths(n)             → 預計算每個控制點的弧長比例 (0~1)
    └─ _globalCurveArcLengths[]  → 快取弧長映射表

sampleGlobalCurve(segIdx, alpha) → 從全域曲線取第 segIdx 段的 alpha 位置
