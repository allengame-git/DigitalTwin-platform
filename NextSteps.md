...
**最後更新**: 2026-03-04（設施模型資訊系統 — WYSIWYG + Dashboard + InfoPanel 改寫）
...
### 5. 設施導覽模組 — 完整（含 2026-03-04 模型資訊系統）

#### 5a. 後端與資料庫
- Prisma Schema：`FacilityScene`（自我參照巢狀）+ `FacilityModel`（含 `introduction String?`）+ `FacilityModelInfo`
- 後端 API（`server/routes/facility.ts`）：場景/模型/Rich Content/平面圖/地形 完整 CRUD
- `PUT /api/facility/models/:id`：支援 `introduction` 欄位儲存 WYSIWYG HTML
...
- 側邊欄（FacilitySidebar）：白色亮色主題，與地質側邊欄設計一致
  ...
  - 收合時顯示直排「設施導覽模組」文字
- BreadcrumbNav、SceneTree：inline 亮色 styles
- 編輯模式：點擊模型 → 設定 `editingModelId`（不進入子場景）
- TransformInputPanel：軸標籤正確對應座標系（X東/Z高程/Y北，Ry方位）
- 模型互動：非編輯模式下，點擊有子場景的模型 → 正常選取；sidebar 提供進入子場景入口
- 標籤：點擊可選取模型
- 截圖：右下角按鈕，使用 `showSaveFilePicker`（OS 原生存檔對話框）+ fallback 自動下載

#### 5d. 模型資訊面板（FacilityInfoPanel — 2026-03-04 重新設計）
- **位置**：右下角浮動面板，`width: 340px`，`max-height: 60vh`，白底圓角陰影
- **三區塊**（無資料的區塊自動隱藏）：
  1. **設施介紹**：`RichTextView` 渲染 `model.introduction`（WYSIWYG HTML）
  2. **設施圖說**：IMAGE 縮圖陣列（72x56px，點擊 Lightbox 全螢幕預覽）+ DOCUMENT 下載連結
  3. **自訂欄位**：label + content（TEXT 純文字 / LINK 超連結另開分頁）
- **Lightbox**：固定全螢幕覆蓋，點擊外部關閉
- **Header**：模型名稱（truncate）+ 編輯按鈕（editMode 時顯示）+ X 關閉按鈕

#### 5e. 模型資訊 Dashboard（FacilityUploadSection Tab 3 — 2026-03-04）
- **卡片牆**：選擇場景（含主場景與子場景）後，3 欄 responsive grid 顯示所有模型
- **ModelCard**：
  - 縮圖（取 infos 第一筆 IMAGE；無則顯示預設 box icon）
  - 模型名稱（粗體）
  - 設施介紹前 80 字摘要（HTML tags 移除；無則顯示「尚未填寫介紹」）
  - 底部 badges：「圖說 N」（藍色）、「欄位 N」（綠色），僅 count > 0 時顯示
- **ModelInfoModal**（點卡片後全螢幕 Modal）：
  - Sticky header：模型名稱 + 儲存狀態 + X 關閉
  - **區塊 1 — 設施介紹**：TipTap WYSIWYG 編輯器（Bold/Italic/H2/H3/BulletList/Link），2 秒 debounce 自動儲存 + 手動「儲存介紹」按鈕 → `PUT /api/facility/models/:id { introduction: html }`
  - **區塊 2 — 設施圖說**：拖拉上傳區（JPG/PNG/PDF/CAD/DWG），上傳後卡片顯示（圖片縮圖 / 文件 icon+檔名），可個別刪除 → `DELETE /api/facility/info/:id`
  - **區塊 3 — 自訂欄位**：現有欄位清單（label + content，可刪除）+ 新增區（type dropdown TEXT/LINK + label input + content input）

#### 5f. 共用元件（`src/components/common/`）
- **RichTextEditor**：TipTap WYSIWYG 編輯器，props: `value`, `onChange`, `placeholder`
  - 工具列：Bold / Italic / H2 / H3 / BulletList / Link
  - 外部 value 變更時同步（useEffect deps: `[value]`，非 `[value, editor]` — 避免無限迴圈）
  - Placeholder 以 absolute positioned div 實作
- **RichTextView**：唯讀 HTML 渲染，props: `html`, `style?`
  - 空白 HTML 偵測：regex `/^(<p>(\s|<br[^>]*>)*<\/p>\s*)+$/`（處理 TipTap 的 `<p><br></p>` 等變體）
  - 使用 `dangerouslySetInnerHTML`（封閉管理系統內安全）
- **CSS**：全域 `.ProseMirror` 和 `.rich-text-view` 樣式定義在 `src/index.css`

#### 5g. 設施資料管理（FacilityDataPage / FacilityUploadSection）
- 5 個 Tab：場景管理 / 模型上傳 / **模型資訊**（Dashboard 卡片牆） / 場景地形 / **模型管理**
- Tab 3「模型資訊」（ModelInfoDashboard）：場景選擇器（全場景可選）→ 卡片牆 → Modal 編輯
- Tab 5「模型管理」（ModelManager）：
  - 選擇場景後列出模型卡片（可展開/收合）
  - 展開後可編輯：名稱、子場景、位置、旋轉、縮放
  - 「儲存變更」/ 「刪除模型」
...
3. **自訂欄位 API**：`handleAddField` 使用 `FormData`（multipart）送純文字，可改用 JSON（`application/json`）。需確認後端 `/api/facility/models/:id/info` 是否同時接受 JSON 與 FormData。
...
| 9 | `(s as any).parentSceneId` 型別 cast | 低 | `FacilityUploadSection` 中 `FacilityScene` 存取 `parentSceneId` 需 `as any`，應確認型別定義是否完整 |
...
| `FacilityInfoPanel.tsx` | **右下角浮動面板**（介紹/圖說/自訂欄位，340px/60vh） |
...
| 檔案 | 說明 |
|:-----|:-----|
| `RichTextEditor.tsx` | TipTap WYSIWYG 編輯器（Bold/Italic/H2/H3/BulletList/Link） |
| `RichTextView.tsx` | WYSIWYG HTML 唯讀渲染（含空白 HTML 偵測） |
...
| 檔案 | 說明 |
|:-----|:-----|
| `src/types/facility.ts` | `FacilityScene`, `FacilityModel`（含 `introduction?`）, `FacilityModelInfo`, `Transform` |
...
2. 確認 Prisma schema 已 `npx prisma db push` 同步（`introduction String?` 欄位）
