...
### 5. 設施導覽模組 — 完整（含動畫系統 + 曲線路徑 + 3D 路徑編輯 + 手動觸發 + 場景轉場 + Error Boundary + 視角切換 + 地形三模式 + 框選多選 + 動畫匯出匯入 + N 級場景樹 + 平面圖標記編輯）
...
// 地形設定
terrainSettings: FacilityTerrainSettings   // visible, textureMode, colorRamp, reverse, minZ, maxZ, autoRange
setTerrainSettings(patch)                  // Partial update
...
setSelectedModelIds(ids, focusId?)  // 框選批次設定（marquee select）
...
#### 5e-2. 框選多選 Marquee Selection（2026-03-10）

- **`useMarqueeSelect` hook**（`src/hooks/useMarqueeSelect.ts`）：Canvas wrapper div 監聯 pointer 事件，拖曳 > 5px 啟動框選
- **Module-level refs**：`_setMarqueeRefs(camera, controls)` 由 Canvas 內 `MarqueeCameraSync` 設定
- **Hit test**：`getModelGroupRef()` + `Box3.setFromObject()` 取即時世界座標（非 stale `modelBboxCenters`），`Vector3.project(camera)` 投影到螢幕座標
- **MapControls 共存**：框選中 disable controls，放開 re-enable；< 5px 不觸發
- **CMD/Ctrl**：不按 = 取代選取，按住 = 追加（union）
- **`MarqueeOverlay`**（`src/components/facility/MarqueeOverlay.tsx`）：半透明藍色矩形 overlay
- **`FacilityCanvas`**：`onPointerMissed` 加 `isDragging.current` guard
- **僅限編輯/動畫模式**：一般瀏覽不啟用

#### 5e-3. 設施地形三模式紋理 + 顯示開關（2026-03-10）

- **`FacilityTerrainSettings`**（`facilityStore.ts`）：`visible`, `textureMode`（satellite/hillshade/colorRamp）, `colorRamp`, `reverse`, `minZ`, `maxZ`, `autoRange`
- **`FacilityTerrain.tsx`**：
  - 衛星影像：從 DB `terrainTextureUrl` 載入
  - 山影圖：從 `heightmapUrl.replace('heightmap.png', 'texture.png')` 推導
  - 色階：`onBeforeCompile` 注入 GLSL shader（`uRamp/uMinZ/uMaxZ/uUseColorRamp` uniforms），`useFrame` 每幀更新
  - `key={meshKey}` 強制 remount 確保 shader 重新編譯
  - `terrainSettings.visible === false` 時 return null
- **`TerrainSettingsSection`**（`FacilitySidebar.tsx`）：
  - 紋理模式按鈕組（有衛星才顯示衛星選項）
  - 色階：顏色映射 select（5 種）、反轉 checkbox、min/max Z 輸入、自動範圍、漸層預覽條
  - 眼睛按鈕：地形顯示/隱藏開關
  - 僅在場景有地形時顯示

#### 5e-4. 地形管理功能（2026-03-10）

- **`FacilityUploadSection.tsx`**：
  - 地形上傳後顯示地形資訊（bounds/heightmap URL）
  - Shift/Rotation 編輯（呼叫 `updateScene` API）
  - 刪除確認（呼叫 DELETE endpoint，刷新 scenes）

#### 5e-5. Bug 修復（2026-03-10）

- **附屬模型框選**：`bboxCenterReported` 邏輯移到 label check 之前，確保無 label 的裝飾模型也能註冊 bbox center
- **動畫附屬模型框選**：`performHitTest` 改用 `getModelGroupRef()` + `Box3` 即時計算，不依賴 stale `modelBboxCenters`
- **頁面 reload 跳登入**：`checkAuth()` refresh 後呼叫 `getCurrentUser()` + 設定 `isLoading: false`；`auth.ts` circular dependency 改用 lazy `await import()`
- **地形上傳 "Unexpected field"**：`fd.append('csv', ...)` → `fd.append('file', ...)`
- **地形 collinear data**：padding zero-range axes + `nearest` fallback
- **衛星影像同檔錯誤**：`os.path.abspath` equality check before `shutil.copy2`
- **衛星影像無法載入**：GeoTIFF 直接 copy 改為 rasterio reproject + JPEG 輸出
- **設施地形渲染錯誤**：Canvas 2D 降級 16-bit → 改用 Three.js native `displacementMap`
- **編輯/動畫模式 InfoPanel**：加 `editMode || animationMode` guard 隱藏
- **Hooks 順序錯誤**: `TerrainSettingsSection` 的 `return null` 在 `useEffect` 之前，切換無地形場景 crash → 移到 hooks 之後
- **projectStore 缺認證**: 安全修復加 `authenticate` 後 `projectStore.ts` 的 fetch 回 401 → 加 `authHeaders()`

#### 5e-6. 平面圖標記編輯（2026-03-10）

- **DB Schema**: `FacilityModel` 新增 `planX Float?`、`planY Float?`、`planVisible Boolean @default(true)`
- **API**: `PUT /api/facility/models/:id/plan-marker`（`server/routes/facility.ts`），驗證 0~100
- **型別**: `FacilityModel` interface 新增三欄位（`src/types/facility.ts`）
- **facilityStore**: 新增 `setModels` action 供拖曳時樂觀更新
- **PlanViewFloating.tsx**: 編輯模式（右上角 toggle）、拖曳標記、眼睛可見性按鈕、半透明隱藏標記、debounce 300ms API 寫入
- **PlanView.tsx**: 讀取 `planX`/`planY` 自訂位置 + `visibleModels` 篩選（decorative/hidden）
- **FacilityPage.tsx**: Lobby 場景左下角「平面圖」開啟按鈕
- **設計文件**: `docs/plans/2026-03-10-plan-view-marker-editing-design.md`
- **Bug 修復 (2026-03-12)**:
  - `updateModelLocal`/`handlePointerUp` 改用 `useFacilityStore.getState().models` 避免 stale closure
  - `toggleVisibility` rollback 用呼叫時捕捉的 `oldVisible` 而非 closure 中的 model
  - API `planVisible` 欄位新增 `typeof boolean` 型別驗證
  - 元件 unmount 時 `clearTimeout(debounceRef.current)` 清理
...
- 場景管理新增「場景範圍」欄位（寬/深/高，選填，空白=自動計算），用於自適應燈光系統
...
**CameraController** 監聯 `viewPreset` 變化 → 計算目標相機位置 → 800ms cubic ease-out fly 動畫

#### 5l. 自適應燈光系統（2026-03-06）

**目的**：場景範圍 10m~10km 差異巨大，固定燈光參數無法兼顧。

**DB Schema**：`FacilityScene` 新增 `sceneBounds Json?`（`{ width, depth, height }` 米），null = 自動計算。

**`FacilityEnvironment.tsx` — `useSceneLighting()` hook**：
- 優先使用手動設定的 `sceneBounds`
- Fallback：根據 `modelBboxCenters` 計算所有模型 bounding box 最大跨距 + 模型 scale buffer
- 最小 range 保護：`Math.max(range, 20)`

**動態計算公式**（`range = max(width, depth)`）：

| 參數 | 公式 |
|------|------|
| 光源位置 | `[range*0.3, range*0.5, -range*0.2]` |
| shadow camera 範圍 | `+-range * 0.7` |
| shadow map | 固定 4096（GPU 安全值） |
| shadow camera far | `range * 2` |
...
**設計文件**: `docs/plans/2026-03-06-adaptive-lighting-design.md`

#### 5m. 子場景自動 fit-all 視角（2026-03-06）

**問題**：子場景通常沒有設定 `cameraPosition`，進入後相機停在原位不動。

**修復**（`FacilityCameraController.tsx`）：
- 新增 `useEffect` 監聽 `currentScene.id` + `modelBboxCenters`
- 條件：`!currentScene.cameraPosition && modelBboxCenters 有資料 && 尚未 fit 過`
- 計算：所有模型 bbox 中心平均 → 目標點，估算場景範圍 → 相機距離
- transition 黑幕中：瞬間設定相機；一般切換：800ms 飛行動畫
- `autoFitDoneForScene` ref 防止同一場景重複 fit

#### 5n. ghostMode 路徑節點拖曳改善（2026-03-06）

**問題**：動畫模式下點擊路徑節點，模型因 `interpolateKeyframes` 移動到該 keyframe 位置，遮擋節點球無法拖曳。

**修復**（`FacilityModelItem.tsx`）：
- `isEditingPathNode = editingKeyframeIndex !== null && animationMode`
- ghostMode 進入：`userData._origOpacity/._origTransparent` 保存原始值，`mat.opacity = 0.3`, `mat.transparent = true`, `mesh.raycast = () => {}`
- ghostMode 退出：從 userData 恢復原始值，刪除暫存 key
- 每次都呼叫 `mat.needsUpdate = true`

**設計文件**: `docs/plans/2026-03-06-path-node-drag-ux-design.md`
...
### 8. 正式帳號管理系統 (2026-03-09) — 完整

#### 8a. 後端
- **Prisma Schema**: `User`（UserRole/AccountStatus enums, bcrypt passwordHash, failedLoginCount, lockedUntil, mustChangePassword）+ `Session`（refreshToken unique, isRevoked, expiresAt）+ `AuditLog`（AuditAction enum 11 種事件）
- **認證 API** (`server/routes/auth.ts`): login（帳號鎖定/自動解鎖/Session 併發控制 max 3）、logout、refresh、me、change-password
- **登入**: 支援 email 或使用者名稱（自動判斷 `@` 符號，email 用 `findUnique`、name 用 `findFirst`）
- **Admin API** (`server/routes/admin.ts`): 9 個端點，全部 `authenticate + authorize('admin') + adminRateLimit`
- **Rate limiting** (`server/middleware/rateLimit.ts`): in-memory sliding window，3 層級
- **CSRF** (`server/middleware/csrf.ts`): Double Submit Cookie，admin 路由掛載 `verifyCsrf`
- **密碼規則** (`server/lib/passwordPolicy.ts`): 8+字元/大寫/小寫/數字，不可與 email 相同
- **稽核日誌** (`server/lib/auditLog.ts`): 寫入失敗不阻斷主流程
- **Seed** (`server/prisma/seed.ts`): `admin@llrwd.tw` / `Admin@2026`，`mustChangePassword: true`

#### 8b. 前端
- **型別** (`src/types/auth.ts`): AdminUser, CreateUserRequest/Response, AuditLogEntry, AuditLogFilters, PaginatedResponse<T>, UserSession
- **API** (`src/api/auth.ts`): fetchApi 自動注入 Bearer token + CSRF token
- **API** (`src/api/admin.ts`): adminFetch helper + 9 個管理員 API 函數
- **Store** (`src/stores/adminStore.ts`): users/auditLogs/userSessions 非同步管理
- **Store** (`src/stores/authStore.ts`): 新增 `mustChangePassword` + `changePassword` action
- **ChangePasswordPage**: 密碼強度即時檢查清單、3 個密碼欄位顯示/隱藏切換、首次登入強制導向
- **AdminUsersPage**: 真實 API 使用者列表、搜尋/角色/狀態篩選、操作選單（編輯/重設密碼/解鎖/停用）、臨時密碼複製
- **AdminSettingsPage**: 5 張安全資訊卡片 + 稽核日誌檢視器（日期/事件篩選、分頁）
- **LoginForm**: 支援使用者名稱或 email 登入（`type="text"`）
...
#### Important — 第二輪修復（2026-03-06，已完成）

| # | 問題 | 修復方式 | 檔案 |
|---|------|---------|------|
| I1-NEW | **useFrame GC 壓力** — `interpolateKeyframes` 每幀 new 6~12 個 THREE 物件 | 14 個 module-level 可重用物件（`_posA/_posB/_posResult/_q1/_q2/...`） | `FacilityModelItem.tsx` |
| I2-NEW | **檔案刪除路徑穿越** — DB 儲存 URL 拼接成絕對路徑可跳脫 uploads/ | 新增 `server/lib/safePath.ts`，`safeResolvePath()` 驗證路徑 | `facility.ts`, `borehole.ts`, `water-level.ts` |
| I3-NEW | **動畫 API 無所有權驗證** — 任何已認證使用者可改刪任意動畫 | `verifyAnimationOwnership()` 檢查 animation→model→scene→project 鏈 | `facility.ts` |
| I4-NEW | **GET 路由缺認證** — facility 所有 GET 完全公開 | scenes/models/info/animations 加 `authenticate` middleware | `facility.ts` |

#### Important（尚未修復 — 需評估）

| # | 問題 | 建議 | 優先級 |
|---|------|------|--------|
| ~~I1~~ | ~~**認證系統 in-memory** — server 重啟遺失使用者~~ | ~~已完成 (2026-03-09)：遷移至 Prisma User/Session/AuditLog~~ | ~~已修復~~ |
| ~~I3~~ | ~~**其他模組 GET 路由無認證** — borehole/terrain/geology-model 等仍公開~~ | ~~已完成 (2026-03-10)：8 個路由檔案共 19 個路由加上 `authenticate`~~ | ~~已修復~~ |

#### 第三輪安全發現（2026-03-08，尚未修復）

| # | 問題 | 說明 | 優先級 |
|---|------|------|--------|
| ~~V3-1~~ | ~~**`safeResolvePath()` 永遠回傳 null**~~ | ~~已修復 (2026-03-10)：strip 前導 `/` 再 `path.resolve(SERVER_ROOT, relative)`，DB URL 正確 resolve~~ | ~~已修復~~ |
| ~~V3-2~~ | ~~**terrain.ts 檔案刪除無路徑驗證**~~ | ~~已修復 (2026-03-10)：改用 `safeResolvePath()` 驗證每個 URL~~ | ~~已修復~~ |
| ~~V3-3~~ | ~~**geology-model.ts 遞迴刪除無邊界**~~ | ~~已修復 (2026-03-10)：`safeResolvePath()` + 額外檢查 parentDir 必須在 `uploads/geology-tiles` 下~~ | ~~已修復~~ |
| ~~V3-4~~ | ~~**lithology.ts / project.ts POST/PUT/DELETE 缺 authenticate**~~ | ~~已修復 (2026-03-10)：所有路由加 `authenticate`，含 GET/POST/PUT/DELETE~~ | ~~已修復~~ |
| ~~V3-5~~ | ~~**auth.ts 硬編碼預設帳號**~~ | ~~已修復 (2026-03-09)：移除 in-memory 硬編碼帳號，改用 Prisma seed (`admin@llrwd.tw`)~~ | ~~已修復~~ |

**V3-1~V3-4 已修復**（2026-03-10）：
- `safePath.ts`：strip 前導 `/`，`path.resolve(SERVER_ROOT, relative)` 正確 resolve DB URL
- `terrain.ts` DELETE：改用 `safeResolvePath()` 驗證後才 unlink
- `geology-model.ts` DELETE：`safeResolvePath()` + `parentDir.startsWith(uploads/geology-tiles)` 雙重驗證
- `lithology.ts` / `project.ts`：所有路由加 `authenticate`

#### Suggestions（建議改善）

| # | 問題 | 建議 |
|---|------|------|
| ~~S1~~ | ~~`useFrame` 每幀 `new THREE.Vector3` 產生 GC 壓力~~ | ~~已修復 I1-NEW~~ |
| ~~S2~~ | ~~`setPlaybackTime` 每幀觸發 Zustand 更新~~ | ~~已修復 (2026-03-10)：值變化 > 0.016 才更新 store~~ |
| ~~S3~~ | ~~FacilityCanvas 缺頂層 ErrorBoundary~~ | ~~已修復 (2026-03-10)：包裹 ErrorBoundary + 3D 場景專用 fallback~~ |
| ~~S4~~ | ~~Python 路徑僅限 macOS/Linux~~ | ~~已修復 (2026-03-10)：新增 `lib/pythonPath.ts`，支援 Windows `Scripts\\python.exe`~~ |
...
### ~~A5. 動畫匯出/匯入~~ — 已完成 (2026-03-10)

**實作摘要**：
- **匯出**: 單一動畫或整個模型的所有 keyframe 動畫匯出為 JSON（版本 1 格式，含 metadata）
- **匯入**: 選擇 JSON 檔案 → 驗證格式 → 對所有選取模型批次匯入，自動計算 position offset 對齊
- **UI**: 屬性面板新增 Download（單一）+ Download+Plus（全部）圖示按鈕；Header 新增 Upload 匯入按鈕
- **型別**: `AnimationExportData` + `AnimationExportFile` (`src/types/facility.ts`)
- **設計文件**: `docs/plans/2026-03-10-animation-export-import-design.md`
...
### ~~SEC1. 認證系統遷移至 DB（對應 I1）~~ — 已完成 (2026-03-09)

已遷移至 Prisma `User`/`Session`/`AuditLog` 模型，含 bcrypt 密碼雜湊、refresh token 持久化、admin CRUD API、rate limiting、CSRF。詳見上方第 8 節。

---

### ~~SEC2. API 路由認證策略統一（對應 I3）~~ — 已完成 (2026-03-10)

所有模組 GET/POST/PUT/DELETE 路由已統一加上 `authenticate` middleware。

**已修復路由檔案**（共 8 個，19 個路由）：
- `borehole.ts` — GET `/`, GET `/:id`
- `terrain.ts` — GET `/`
- `geology-model.ts` — GET `/`, GET `/:id`, GET `/:id/status`
- `upload.ts` — GET `/imagery`, GET `/imagery/:id`, GET `/geophysics`, GET `/geophysics/:id`
- `faultPlane.ts` — GET `/`, GET `/:id`
- `attitude.ts` — GET `/`
- `project.ts` — GET `/`, GET `/:id`, GET `/code/:code`, GET `/:id/stats`, POST `/`, PUT `/:id`, DELETE `/:id`
- `lithology.ts` — GET `/`, POST `/`, PUT `/:id`, DELETE `/:id`, POST `/init-defaults`, POST `/batch`

**唯一保持公開**：`invite.ts` 的 `GET /:token/info`（受邀者登入前查詢邀請資訊）

---

### ~~SEC3. 效能優化（對應 S2, S3）~~ — 已完成 (2026-03-10)

~~**S1 — useFrame 記憶體分配**: 已修復（I1-NEW），14 個 module-level reusable 變數。~~

~~**S2 — setPlaybackTime 節流**: 已修復。`FacilityModelItem.tsx` 的 `useFrame` 中，比較 `currentTime` 與前值差距 > 0.016 才呼叫 `setPlaybackTime()`，避免每幀觸發 Zustand re-render。~~

~~**S3 — FacilityCanvas ErrorBoundary**: 已修復。`FacilityCanvas.tsx` 外層包裹 `ErrorBoundary`，含 3D 場景專用 fallback（錯誤訊息 + 重新載入按鈕），防止 WebGL context 錯誤白屏。~~

~~**S4 — 跨平台 Python 路徑**: 已修復。新增 `server/lib/pythonPath.ts`，`getPythonExecutable()` 依序檢查 Unix venv → Windows venv → 系統 Python，`facility.ts`/`water-level.ts` 統一改用。~~

---

### SEC4. 場景管理 N 級巢狀遞迴樹 — 已完成 (2026-03-10)

**問題**: SceneManager 只能新增主場景的直接子場景，模型下拉只顯示主場景模型。

**修復**: 重構 `FacilityUploadSection.tsx` 的 SceneManager：
- `buildSceneTree()` 從 flat scenes array 建構遞迴樹
- `SceneTreeNode` 遞迴元件：每個節點載入自身模型供子節點 ModelSelect、動態 `parentSceneId`、展開/收合
- `countDescendants()` 刪除時提示後代數量
- 設計文件: `docs/plans/2026-03-10-subscene-tree-design.md`
...
#### ~~G1. 岩性系統整合~~ ✅ 已完成 (2026-03-12)

- ~~確認 3D 地質模型渲染使用專案級 `LithologyDefinition` 顏色~~ → `GeologyTiles.tsx` 已用 `useLithologyStore` 建 palette
- ~~岩性顏色編輯後，3D 場景即時更新~~ → display mesh `onBeforeCompile` + cap material `useEffect` 同步 uniforms
- ~~CSV Voxel `lith_id` 與 `LithologyDefinition` ID 對應~~ → Python vertex color R channel → shader 查表 `uLithIds[]`
- **根因修復**: `lithologyStore`/`attitudeStore`/`boreholeStore`/`faultPlaneStore` GET 請求缺 auth header → 401 → lithologies 空陣列 → 全部 fallback 灰色

#### ~~G2. 斷層面功能完善~~ ✅ 已完成 (2026-03-12)

- ~~3D 視覺增強：斷層名稱標籤（Html Label）~~ → `StructureLines.tsx` 加 `<Html>` 浮動標籤，含名稱+類型+斷層色帶
- ~~傾向指示箭頭~~ → 用戶確認不需要，跳過
- ~~點擊斷層後側邊欄自動顯示詳細參數~~ → 既有功能確認正常（`InspectorPanel.tsx` → `FaultSection()`）
...
| 11 | ~~場景無 `cameraPosition` 時視角不動~~ | ~~中~~ | ~~已修復：子場景 auto-fit + 自適應燈光，無 cameraPosition 時自動計算 fit-all 視角~~ |
...
| ~~17~~ | ~~認證系統 in-memory~~ | ~~中~~ | ~~已修復 (2026-03-09)：遷移至 Prisma User/Session/AuditLog~~ |
| 18 | ~~facility GET 路由無認證~~ | ~~中~~ | ~~已修復 I4-NEW：scenes/models/info/animations 加 authenticate~~ |
| ~~18b~~ | ~~其他模組 GET 路由無認證~~ | ~~中~~ | ~~已修復 I3 (2026-03-10)：8 檔案 19 路由加 authenticate~~ |
| 19 | ~~`useFrame` 每幀分配 THREE 物件~~ | ~~低~~ | ~~已修復 I1-NEW：14 個 module-level reusable 變數~~ |
| ~~20~~ | ~~`setPlaybackTime` 每幀 Zustand 更新~~ | ~~低~~ | ~~已修復 S2 (2026-03-10)：值變化 > 0.016 才更新~~ |
| ~~21~~ | ~~FacilityCanvas 缺頂層 ErrorBoundary~~ | ~~中~~ | ~~已修復 S3 (2026-03-10)：ErrorBoundary + fallback~~ |
| ~~22~~ | ~~Python 路徑僅限 macOS/Linux~~ | ~~低~~ | ~~已修復 S4 (2026-03-10)：`lib/pythonPath.ts` 跨平台~~ |
| 23 | `express.json limit: 50mb` | 低 | S5：潛在記憶體消耗攻擊向量 |
| 24 | 使用者名稱登入可能不唯一 | 中 | Prisma `User.name` 欄位無 `@unique`，`findFirst` 可能取到第一個同名使用者。建議加 `@unique` 或改用專屬 `username` 欄位 |
| 25 | Rate limiting in-memory 不跨實例 | 低 | 多 server 實例時 rate limit 無法共享，production 需改用 Redis |
...
| `server/routes/auth.ts` | **認證 API** (Prisma 持久化, login 支援 email/username, 帳號鎖定, Session 併發控制) |
| `server/routes/admin.ts` | **管理員 API** (使用者 CRUD, 重設密碼, 解鎖, 停用, Session 管理, 稽核日誌) |
| `server/middleware/rateLimit.ts` | **Rate limiting** (in-memory sliding window, 3 層級) |
| `server/middleware/csrf.ts` | **CSRF 保護** (Double Submit Cookie) |
| `server/lib/passwordPolicy.ts` | **密碼規則** (驗證 + 臨時密碼產生) |
| `server/lib/auditLog.ts` | **稽核日誌寫入** (11 種事件, 失敗不阻斷主流程) |
...
| `server/lib/safePath.ts` | **路徑穿越防護** — `safeResolvePath()` 共用工具（I2-NEW） |
| `server/lib/pythonPath.ts` | **跨平台 Python 路徑** — `getPythonExecutable()` 偵測 venv（S4） |
...
| `FacilityEnvironment.tsx` | **自適應燈光** — `useSceneLighting()` hook 依場景範圍動態計算光源/shadow/grid/fog |
| `FacilityCameraController.tsx` | 800ms cubic ease-out fly-to 動畫 + **視角快速切換** + **場景轉場相機瞬移** + **子場景 auto-fit** |
...
| `PlanView.tsx` | 2D 平面圖 + 模型標記（讀取 `planX`/`planY` 自訂位置 + decorative/hidden 篩選） |
| `PlanViewFloating.tsx` | 浮動平面圖容器 + **編輯模式**（拖曳標記位置 + 眼睛可見性 toggle + debounce API 寫入） |
...
| `src/stores/authStore.ts` | **認證狀態** (JWT, login/logout, mustChangePassword, changePassword) |
| `src/stores/adminStore.ts` | **管理員** (使用者 CRUD, 稽核日誌, session 管理) |
| `src/api/auth.ts` | **認證 API client** (fetchApi with Bearer + CSRF injection) |
| `src/api/admin.ts` | **管理員 API client** (adminFetch helper + 9 functions) |
| `src/types/auth.ts` | **認證型別** (AdminUser, AuditLogEntry, PaginatedResponse, etc.) |
...
| 檔案 | 說明 |
|:-----|:-----|
| `src/types/facility.ts` | `FacilityScene`, `FacilityModel`（含 `planX`/`planY`/`planVisible`）, `FacilityAnimation`（含 pathMode/autoOrient）, `AnimationKeyframe`（含 per-segment pathMode）, `FacilityModelInfo`, `Transform` |
...
| `docs/plans/2026-03-06-path-node-drag-ux-design.md` | **ghostMode 路徑節點拖曳 UX 設計文件**（已完成） |
| `docs/plans/2026-03-06-adaptive-lighting-design.md` | **自適應燈光系統設計文件**（已完成） |
